class Search::Query
  ...
  def initialize(*options)
    ...
    if options.size == 0
      ...
      @query = { query: filter_query(@filters) }
    end
    build_query *options
  end
    ...
    params.each do |field, value|
      if field == 'cpvs'
        ...
        value.each do |cpv_prefix|
          cpv_queries << create_filter(FIELDS[field], cpv_prefix, 'prefix_query')
        end
        @filters << should_query(cpv_queries)
      else
  ...
  def create_filter field, value, query_type='terms_query'
    ...
    complete_query = nested_query(subject, filter_query(Array.new().push(query)))
